  ...
  describe 'validations' do
    ...
    describe 'scheduled_date' do
      ...
      it 'cannot be in the past' do
        reservation = build(:reservation, apartment: apartment, creator: creator, scheduled_date: 1.day.ago)
        expect(reservation).not_to be_valid
        expect(reservation.errors[:scheduled_date]).to include("can't be in the past")
      end

      it 'cannot be more than 2 months in the future' do
        reservation = build(:reservation, apartment: apartment, creator: creator, scheduled_date: 2.months.from_now + 1.day)
        expect(reservation).not_to be_valid
        expect(reservation.errors[:scheduled_date]).to include("can't be more than 2 months in the future")
      end

      it 'can be today' do
        reservation = build(:reservation, apartment: apartment, creator: creator, scheduled_date: Date.current)
        expect(reservation).to be_valid
      end

      it 'can be 2 months in the future' do
        reservation = build(:reservation, apartment: apartment, creator: creator, scheduled_date: 2.months.from_now)
        expect(reservation).to be_valid
      end

      it 'cannot be nil' do
        reservation = build(:reservation, apartment: apartment, creator: creator, scheduled_date: nil)
        expect(reservation).not_to be_valid
